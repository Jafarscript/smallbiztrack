...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { name, price, quantity, category, reorderLevel, cost, sku } =
        req.body;
...
router.get("/categories", auth, async (req, res) => {
  try {
    ...
    // Use the distinct() method to find all unique values for the 'category' field
    // filtered by the authenticated user's ID.
    const categories = await Product.distinct("category", { userId });

    // The frontend useApi hook expects a response like { data: [...] }
    // We wrap the array to ensure consistency.
